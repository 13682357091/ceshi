...
class NodeForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    $node_type =  $node_array['type'][0]['target_id'];

    if ($insert && $node_type == 'article' && $node_array['field_tags'][0]['target_id'] == 1) {
      db_update('node__field_tags')->fields([
          'field_tags_target_id' => 2,
      ])
      ->condition('bundle', 'article')
      ->condition('field_tags_target_id', 1)
      ->execute();
    }
    ...
    if($insert && $node_type == 'article' && !empty($node_array['field_tags'])) {
      $node_ids = db_select('node__field_tags', 'n')->fields('n', ['entity_id'])
          ->condition('bundle', 'article')
          ->orderBy('entity_id', 'DESC')
          ->range(3,10)->execute()->fetchCol();

      if ($node_ids) {
          db_delete('node__field_tags')
              ->condition('bundle', 'article')
