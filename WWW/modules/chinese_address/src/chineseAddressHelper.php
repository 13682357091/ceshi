...
class chineseAddressHelper
{
   ...
   */
    public static function chinese_address_get_location($parentId = self::CHINESE_ADDRESS_ROOT_INDEX, $excludeNone = false, $limitIds = array()) 
    {
        ...
        if (!$excludeNone) {
            $result[self::CHINESE_ADDRESS_NULL_INDEX] = '选择省/直辖市/自治区';
            ksort($result);
        }
        return $result;
    }
   ...
   */
    ...
    public static function _chinese_address_get_name($regionId)
    {
        $entries = self::_chinese_address_get_parents($regionId, 3);
        ...
        if ($entries) {
            ...
            foreach ($entries[0] as $ent) {
                $connection =\Drupal::database();
                $name = $connection->select('chinese_address', 'c')->fields('c', array('name'))->condition('id', $ent, '=')->execute()->fetchField();
                $address[$i] = $name;
                $i++;
            }

        }
        $address_str = implode('', array_reverse($address));
